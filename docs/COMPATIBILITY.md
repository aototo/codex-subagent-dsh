...
记录日期：2026-09-20。本文记录源码、本机安装和 CLI 调用结果；不代表已经完成 GitHub 来源的干净安装或桌面端完整验收。
...
- 干净用户环境安装、更新与卸载（本机 local marketplace 首次安装已通过）
...
恢复增量的本地验证快照版本为 `0.1.0+codex.20260919062647`；公开清单版本为 `0.1.0`。类型检查与 60 项自动化测试通过；插件清单和技能校验通过。安装缓存的运行时、技能与清单均与对应验证快照一致。已有对话的 MCP 可能仍运行旧快照，使用新对话加载更新版本。
