...
function view(task: TaskRecord) {
  const { inputHash, ownerId, ownerPid, input, ...publicTask } = task;
  return { ...publicTask, mode: input.mode, baselineCommit: input.baselineCommit, acceptance: 'not_assessed_by_plugin' };
}
